...
def set_performances(l_competitors,l_performances):
    ...
    while not list1.is_empty(l_performances):
        ...
        if get_bibnum(list1.head(l_temp)) == int(list1.head(l_performances)[0]):
            #Vérifie le num du candidat avec le numéro de la performance
            l_competitor_with_performances = list1.cons(list1.head(l_temp),l_competitor_with_performances)
            set_perf(list1.head(l_competitor_with_performances),list1.head(l_performances)[1])
